from qdmr_grounding import QDMRGrounding, is_reference
from qdmr_identifier import QDMRStep
...
class GroundedQDMR(object):
    ...
    def filter_to_sql(self, step):
        ...
        if len(args[1:]) == 1 and len(args[1].split()) == 1:
            ...
            handled_subquery_conds = []
            for cond in filter_conditions:
                cond_column, cond_comparator, cond_val = cond
                if is_reference(cond_val):
                    # (2) compared value is a reference to subquery
                    cond_val = "( %s )" % self.sql_steps[get_ref_number(cond_val)]["SQL"]
                handled_subquery_conds += [(cond_column, cond_comparator, cond_val)]
            filter_conditions = handled_subquery_conds
